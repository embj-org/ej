...
use ej_web::ejjob::{query_jobs, query_run_result};
use ej_web::prelude::*;
...
async fn handle_message(
    ...
    match message {
        ...
        EjSocketClientMessage::FetchJobs(query) => {
            let jobs = query_jobs(&query, &dispatcher.connection)?;
            send_message(writer, EjSocketServerMessage::Jobs(jobs.collect())).await
        }

        EjSocketClientMessage::FetchJobResults(query) => {
